...
exports.inviteMember = exports.removeEkubMember = exports.approveMembershipRequest = exports.disbursePayout = exports.approvePayout = exports.executeDraw = exports.rejectContribution = exports.verifyContribution = exports.addEkubMember = exports.assignEkubAdmin = exports.createEkub = void 0;
...
async function checkIsSuperAdmin(uid, authData) {
...
async function checkIsEkubAdminOrSuperAdmin(uid, ekubId, authData) {
...
// ============================================================================
exports.createEkub = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
...
// ============================================================================
exports.assignEkubAdmin = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
    const isSuper = await checkIsSuperAdmin(context.auth.uid, context.auth);
...
// ============================================================================
exports.addEkubMember = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
...
// ============================================================================
exports.verifyContribution = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
...
// ============================================================================
exports.executeDraw = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (existingDraw.exists) {
            throw new functions.https.HttpsError('already-exists', `Cycle ${cycleNumber} has already been drawn for this Ekub.`);
        }
...
exports.approvePayout = functions.https.onCall(async (data, context) => {
    ...
    const payoutData = await db.runTransaction(async (transaction) => {
        ...
        if (data.status !== 'documents_required' && data.status !== 'under_review') {
            throw new functions.https.HttpsError('failed-precondition', `Payout is currently '${data.status}' and cannot be approved from this state.`);
        }
...
exports.disbursePayout = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
    ...
    const payoutData = await db.runTransaction(async (transaction) => {
        ...
        if (data.status !== 'approved') {
            throw new functions.https.HttpsError('failed-precondition', `Payout is currently '${data.status}' and must be 'approved' before it can be disbursed.`);
        }
        transaction.update(payoutRef, {
            status: 'paid',
            paymentReference,
...
// ============================================================================
exports.approveMembershipRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
...
// ============================================================================
exports.removeEkubMember = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated.');
    }
    const { ekubId, userId } = data;
    ...
    const removedMember = await db.runTransaction(async (transaction) => {
        ...
        if (memberData.role === 'admin') {
            throw new functions.https.HttpsError('failed-precondition', 'The current Ekub Admin cannot be removed as a member -- reassign the Ekub Admin first.');
        }
...
exports.inviteMember = functions.https.onCall(async (data, context) => {
    ...
    if (!isSuper && !isAnyEkubAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only the Super Admin or an Ekub Admin can invite new members.');
    }
    let userRecord = null;
    try {
        ...
        if (err?.code === 'auth/user-not-found') {
            try {
                ...
                if (createErr?.code === 'auth/email-already-exists') {
                    ...
                    throw new functions.https.HttpsError('invalid-argument', createErr?.message || 'Failed to create user account with the provided email.');
                }
            }
        }
        else {
            console.error('Error looking up user by email:', err);
            throw new functions.https.HttpsError('internal', err?.message || 'Failed to look up user account.');
        }
    }
    if (!userRecord) {
    ...
    if (existingProfile.exists) {
        throw new functions.https.HttpsError('already-exists', `A YegnaEkub account for ${trimmedEmail} already exists.`);
    }
    const newProfile = {
        uid: userRecord.uid,
